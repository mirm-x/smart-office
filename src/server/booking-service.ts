...
export async function createBooking(input: CreateBookingInput): Promise<CreateBookingResult> {
  ...
  try {
    ...
    // Resolve any specifically requested resources (from the floor map) to a
    // target id per type. An id that doesn't match an active resource of a
    // requested type is ignored, so the type falls back to auto-assignment.
    ...
    if (input.resourceIds && input.resourceIds.length > 0) {
      ...
      for (const row of requested.rows) {
        if (input.resourceTypes.includes(row.type)) {
