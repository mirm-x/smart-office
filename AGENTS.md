...
- `docs/decisions/README.md` -- all decisions that change or extend the client specification, one section each (e.g. the half-day booking policy). Each record states the rule, why it deviates from spec, and who accepted it. Link to a specific decision by its anchor.
...
- **Booking policy is not yet final**: half-day periods (`src/server/booking-policy.ts`) extend the spec's day-only BR-01/BR-02. `HALF_DAY_BOOKING_ENABLED=true` is the team's POC demo choice, not client acceptance. Do not treat the change as settled until `docs/decisions/README.md#decision-0001-half-day-booking-policy` records a **written** facilitator acceptance. If declined, set the flag to `false` (already wired in `booking-service.ts`) rather than reworking the data model.
- **Scope discipline**: the core POC covers FR-03 (booking), FR-08 (check-in) and FR-09 (automatic release), gated by the tiers in `docs/poc-scope.md`. Employee cancellation of a reserved booking is a requested usability addition; it does not replace those three demo functions. Do not expand into full-delivery features (Entra, notifications, admin console, multi-office) inside the POC codebase, and do not spend camp time on tier-3 items before tier 1 and 2 pass -- describe full-delivery features in the proposal instead.
